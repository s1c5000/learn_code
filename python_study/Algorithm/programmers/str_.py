...
이러한 방식은 반복되는 문자가 적은 경우 압축률이 낮다는 단점이 있습니다. 예를 들면, 
abcabcdede와 같은 문자열은 전혀 압축되지 않습니다. 어피치는 이러한 단점을 해결하기 위해 
...
def solution(s):
    if len(s) ==1:
        return 1
    min = 1001
    for str_size in range(1,int(len(s)/2)+1): # 1부터 len(s)절반까지
        #print("str_size :", str_size)
        arr = []
        for i in range(0,len(s),str_size):
            arr.append(s[i:i+str_size]) # 나누어떨어지지 않는 나머지 무자열은 자동으로 잘려서 붙는다.
        #print(arr)
        ...
        while axis<len(arr):
            i = axis+1
            count = 1
            while i<len(arr) and arr[axis] == arr[i]:
                i += 1
                count +=1
            if count == 1:
                arr2.append(arr[axis])
            else:
                arr2.append(str(count)+arr[axis])
            axis = i 
        arr2 = ''.join(arr2)
        #print(arr2)
        if len(arr2) < min:
